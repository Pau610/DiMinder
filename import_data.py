...
for i, line in enumerate(lines[1:]):
    cols = line.split('\t')
    print(f"Processing line {i+1}: {len(cols)} columns")
    if len(cols) >= 8:
        date_str = cols[0].strip()
        time_str = cols[1].strip()
        glucose_mmol = cols[2].strip()
        long_acting = cols[3].strip()
        short_acting = cols[4].strip()
        timing = cols[5].strip()
        carbs = cols[6].strip()
        food_items = cols[7].strip()
        
        print(f"  Date: '{date_str}', Time: '{time_str}', Glucose: '{glucose_mmol}'")
        ...
        if date_str and time_str:
            try:
                ...
                # Create record for every timestamp entry
                record = {
                    'timestamp': timestamp,
...
if not df.empty:
    ...
    print("No data processed - creating empty DataFrame with correct columns")
    df = pd.DataFrame(columns=['timestamp', 'glucose_level', 'carbs', 'insulin', 'insulin_type', 'injection_site', 'food_details'])
